...
class ProveedorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $clientesEmpresasQuery = Proveedor::query();
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()->back()->withErrors($validator)->withInput()->with('proveedores.create', true);
        }
        ...
        return redirect()->route('proveedores.index')->with('success', 'Cliente creado con éxito.');
    }
    ...
    public function show($id)
    {
        ...
        return view('proveedores.show', compact('cliente'));
    }

    public function getUserData($id)
    {
        $cliente = Proveedor::with('tipoDocumento', 'pais', 'municipio', 'sectorcomercial', 'paisdocumento', 'sucursal', 'idiomas')->find($id);
    ...
    {
        $clientesparticular = Proveedor::findOrFail($id);
        return view('proveedores.edit', compact('clientesparticular'));
    }
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), $rules, $messages);
        if ($validator->fails()) {
            return redirect()->back()->withErrors($validator)->withInput()->with('proveedores.edit', true);
        }
        ...
        if ($request->input('cliente') == '1') {
            ...
            $existingUser = User::where('numero_documento', $request->input('numero_documento'))->first();
            $existingCliente = Cliente::where('numero_documento', $request->input('numero_documento'))->first();
            if (!$existingUser) {
                $user = new User();
        ...
        } elseif ($request->input('cliente') == '0') {
            // Buscar y eliminar el usuario si existe
            $existingUser = User::where('numero_documento', $request->input('numero_documento'))->first();
            if ($existingUser) {
                $existingUser->delete();
            }
    ...
    {
        $cliente = Proveedor::find($id); // Buscar el cliente por ID

        if (!$cliente) {
            return redirect()->route('proveedores.index')->with('error', 'Modelo no encontrado.');
        }
        $existingUser = User::where('numero_documento', $cliente->numero_documento)->first();
        if ($existingUser) {
            $existingUser->delete();
        }
        $existingCliente = Cliente::where('numero_documento', $cliente->numero_documento)->first();
        if ($existingCliente) {
            $existingCliente->delete();
        }
        ...
        return redirect()->route('proveedores.index')->with('success', 'Modelo de vehículo eliminado exitosamente.');
    }
    public function getCiudadesByPais3(Request $request)
    {
